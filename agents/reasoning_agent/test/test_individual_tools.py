...
class TestTools:
            ...
            try:
                ...
                if expected == "error":
                    ...
                    if (
                        ...
                        or result.replace(".", "").replace("-", "").isdigit()
                    ):
                        result_num = (
                            float(result) if isinstance(result, str) else result
                        )
    ...
    def test_date_diff(self):
        ...
        test_cases = [
            ...
            (
                ...
                "2025-07-05",
                1281,
            ),  # Corrected: manual calculation confirms 1281 days
            ...
            (
                ...
                "2024-01-01",
                366,
            ),  # Corrected: should return absolute value
            # Invalid dates
            ...
            try:
                ...
                if expected == "error":
                    ...
                    if str(expected) in result:
                        ...
                        print(
                            f"  ❌ {date1} to {date2} = {result}, expected {expected}"
                        )
        ...
        error_cases = len(
            [tc for tc in test_cases if tc[2] in ["error", "negative_or_error"]]
        )
            ...
            try:
                ...
                if expected == "error":
                    ...
                    if expected in result:
                        ...
                        print(
                            f"  ❌ {code[:30]} -> {result[:50]}, expected '{expected}'"
                        )
    ...
    def test_file_read_write(self):
        ...
        try:
            # Test write_file
            write_result = self.tool_executor.execute(
                "write_file", [temp_file, test_content]
            )
            if (
                "successfully" in write_result.lower()
                or "written" in write_result.lower()
            ):
            ...
            write_error = self.tool_executor.execute(
                "write_file", [invalid_path, "test"]
            )
...
Testing ensures quality."""
        ...
        try:
            ...
            result2 = self.tool_executor.execute(
                "search_text", [temp_file, "NonExistentTerm"]
            )
            if (
                "not found" in result2.lower()
                or "no matches" in result2.lower()
                or len(result2) < 50
            ):
            ...
            result3 = self.tool_executor.execute(
                "search_text", ["non_existent.txt", "test"]
            )
            ...
            result4 = self.tool_executor.execute(
                "search_text", [temp_file, "programming"]
            )
    ...
    def print_summary(self):
        ...
        for tool, result in self.test_results.items():
            ...
            status = (
                ...
                else "⚠️ PARTIAL" if passed_num > 0 else "❌ FAIL"
            )
        ...
        print(
            f"{'OVERALL':20} | {total_passed}/{total_tests} ({overall_percentage:.1f}%) | {'✅ EXCELLENT' if overall_percentage >= 90 else '⚠️ GOOD' if overall_percentage >= 70 else '❌ NEEDS WORK'}"
        )
        ...
        print(
            f"  Status: {'✅ MET' if overall_percentage >= 95 else '⚠️ CLOSE' if overall_percentage >= 85 else '❌ NOT MET'}"
        )

...
def demo_advanced_capabilities():
    ...
    try:
        ...
        print(f"   • Thinking Tools: {capabilities['categories']['thinking_capabilities']}")
        print(f"   • Inference Tools: {capabilities['categories']['inference_capabilities']}")
...
def demo_thinking_tools(executor: AdvancedToolExecutor):
    ...
    test_cases = [
        ...
        ("think_critical", ["AI will replace all jobs", "automation increases", "new jobs created"]),
...
def demo_pattern_analysis(executor: AdvancedToolExecutor):
    ...
    test_cases = [
        ...
        ("pattern_analyze_text", ["file1.txt", "file2.txt", "file3.txt"]),  # File pattern
...
def demo_rag_capabilities(executor: AdvancedToolExecutor):
    ...
    test_cases = [
        ("rag_add_knowledge", ["Python is a programming language known for its simplicity", "demo"]),
        ("rag_add_knowledge", ["Machine learning uses algorithms to find patterns in data", "demo"]),
    ...
    for i, problem in enumerate(complex_problems, 1):
        ...
        try:
            ...
            if hasattr(agent.tool_executor, 'analyze_problem_complexity'):
                ...
                print(f"   • Suggested Tools: {', '.join(analysis['suggested_tools'][:3])}")
    ...
    try:
        ...
        workflow = [
            ...
            },
            {
                ...
                "args": ["X is fibonacci number", "X follows F(n)=F(n-1)+F(n-2)", "0.9"]
            },
            {

...
class AdvancedToolExecutor(ToolExecutor):
    ...
    def _categorize_tools(self):
        ...
        basic_tools = ["date_diff", "run_python", "read_file", "write_file", 
                      "run_shell", "math_calc", "search_text"]
        ...
        # Thinking tools
        thinking_tools = ["think_sequential", "think_systems", "think_critical",
                         "think_lateral", "think_root_cause", "think_six_hats",
                         "get_thinking_summary"]
        self.tool_categories["thinking"] = thinking_tools
        
        # Inference tools
        inference_tools = ["logical_add_rule", "logical_add_fact", "logical_infer",
                          "pattern_analyze_numeric", "pattern_analyze_text", 
                          "inference_status"]
        self.tool_categories["inference"] = inference_tools
        
        # Async capable tools
        async_tools = rag_tools + thinking_tools  # These tools support async
            ...
            if tools:  # Only show categories that have tools
                ...
                for tool in tools:
                    if tool in self.tools:
                        ...
                        async_marker = " [ASYNC]" if tool in self.tool_categories["async"] else ""
    ...
    def analyze_problem_complexity(self, problem_description: str) -> Dict[str, Any]:
        ...
        if any(word in problem_lower for word in ["logic", "prove", "infer", "conclude"]):
            suggested_tools.extend(["logical_add_fact", "logical_infer"])
            complexity_score += 3
        
        if any(word in problem_lower for word in ["research", "knowledge", "search", "information"]):
            suggested_tools.extend(["rag_search", "rag_augmented_query"])
            complexity_score += 2
        
        if any(word in problem_lower for word in ["analyze", "think", "reasoning", "consider"]):
            suggested_tools.extend(["think_sequential", "think_critical", "think_systems"])
            complexity_score += 3
        ...
        return {
            ...
            "requires_async": any(tool in self.tool_categories["async"] 
